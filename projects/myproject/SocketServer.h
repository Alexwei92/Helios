...
private:
        ...
        RadiationModel *radiation_model)
    {
        ...
        if (command == "GET_FRUITS_LOCATIONS")
        {
            ...
            int fruit_id = 0;
            for (int i = 0; i < m_config.plantsUUIDs.size(); i++)
            {
                std::vector<uint> fruit_obj_UUIDs = plantarchitecture.getPlantFruitObjectIDs(m_config.plantsUUIDs[i]);
