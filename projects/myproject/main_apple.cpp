...
#define SAVE_RGB_IMAGE 1     // save rgb images
#define SAVE_DEPTH_IMAGE 1   // save depth images
...
#define SAVE_TO_OBJ 0        // save scene to .obj
...
#define SOCKET_SERVER 1 // enable socket server
...
#define PLANT_COUNT_Y 3
// #define PLANT_AGE 950.0f // days
#define PLANT_AGE 730.0f // days

/** Environment Parameters */
#define SUN_DIR_AZIMUTH deg2rad(45.0)
#define SUN_DIR_ELEVATION deg2rad(60.0)

/** Output Parameters */
// std::string OUTPUT_DIR = "/media/peng/Samsung/Helios_results/apple_trees_five_trees/";
...
int main()
{
    ...
    float wire_radius = 0.0025;
    // RGBcolor wire_color(0.3, 0.3, 0.3);
    RGBcolor wire_color(40.0/255.0, 40.0/255.0, 40.0/255.0);
    ...
    shoot_parameters_proleptic.vegetative_bud_break_probability_min = 0.6;
    shoot_parameters_proleptic.vegetative_bud_break_probability_decay_rate = 0.2;
    ...
    int total_leaves = 0;
    int total_flowers = 0;
    int total_internodes = 0;
    for (uint plant_id : UUIDs_plants)
    {
        total_leaves += plantarchitecture.getPlantLeafObjectIDs(plant_id).size();
        total_flowers += plantarchitecture.getPlantFlowerObjectIDs(plant_id).size();
        total_internodes += plantarchitecture.getPlantInternodeObjectIDs(plant_id).size();
    }
    std::cout << "Number of plants: " << UUIDs_plants.size() << std::endl;
    std::cout << "Number of leaves: " << total_leaves << std::endl;
    std::cout << "Number of flowers: " << total_flowers << std::endl;
    std::cout << "Number of internodes: " << total_internodes << std::endl;
